  ...
  run: async (client, message, args, user, guild) => {
    ...
    if (!user && !user.enrolled) {
      const embed = new MessageEmbed()
        ...
        .setDescription(
          `${message.author.username}, please enroll a course before using this command\nSimply run the (prefix)enroll Command\nExample: **$enroll**`
        );
    ...
    const premiumcourses =
      require("../../questions/courses.json").premium_courses;
    ...
    if (answer === "start") {
      ...
      let finalLevelXP = 0;
      for (let i = 0; i < newLevelXP.length; i++) {
        if (newLevelXP[i][0] && newLevelXP[i][0].xp)
          finalLevelXP = finalLevelXP + newLevelXP[i][0].xp;
      }
        ...
        message.author.send(
          new MessageEmbed()
            ...
            .setDescription(
              ...
              } **(+${newXP} XP)**\n\nTo start the level, make sure you rerun the resume Command!\n\n**Level ${
                info.courses.find(
                  (n) => n.name.toLowerCase() === args[0].toLowerCase()
                ).course.Level
              } Summary**\n- ${newLevelLength} Pages\n- ${finalLevelXP} Total XP`
            )
        );
      } catch {
        return await cancelPrompt(1, message, false);
...
async function cancelPrompt(number, message, answer) {
  ...
  if (number === 1) {
    const cancelEmbed = new MessageEmbed()
      ...
      .setDescription(
        `I couldn't dm you. Please make sure your DMs are open while you use my Commands!`
      );
